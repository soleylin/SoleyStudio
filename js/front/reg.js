...
$(function () {
  ...
  $("#reg_acc").bind("input propertychange", function () {
    if ($(this).val().length > 4 && $(this).val().length < 16) {
      ...
      $.ajax({
        type: "POST",
        url: "https://soleystudio.000webhostapp.com/SoleyStudio/api/member/acc-CheckUni.php",
  ...
  $("#reg_pwd").bind("input propertychange", function () {
    if ($(this).val().length > 2 && $(this).val().length < 16) {
      ...
      if ($("#reg_re_pwd").val() == $(this).val()) {
        ...
        //與確認密碼不同
        $("#reg_re_pwd").removeClass("is-valid");
        $("#reg_re_pwd").addClass("is-invalid");
        flag_re_pwd = false;
      }
    } else {
      //不符合規定
  ...
  $("#reg_tel").bind("input propertychange", function () {
    if ($(this).val().length == 10 && $(this).val().substr(0, 2) == "09") {
      ...
      $.ajax({
        type: "POST",
        url: "https://soleystudio.000webhostapp.com/SoleyStudio/api/member/tel-CheckUni.php",
        data: JSON.stringify(dataJSON),
        dataType: "json",
  ...
  $("#reg_ok_btn").click(function () {
    if (
      ...
      $.ajax({
        type: "POST",
        url: "https://soleystudio.000webhostapp.com/SoleyStudio/api/member/member-Create.php",
        data: JSON.stringify(dataJSON),
        dataType: "json",
        success: showdata,
        error: function () {
