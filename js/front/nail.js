...
function drawTable(page) {
  ...
  newData[page].forEach(function (item) {
    var strHTML =
      '<div class="col-md-6 col-lg-4 col-xl-3"><div class="s2_box ms-auto me-auto"><div class="s2_bgbox"></div><div class="s2_img bg-cover"><a href=" https://soleystudio.000webhostapp.com/image/admin/product/' +
      item.image +
...
function read() {
  newData = [];
  nowpage = 0;
  pData.forEach(function (item, key) {
    if (key % pageNumber == 0) {
      newData.push([]);
      lastpage++;
    }
    var page = parseInt(key / pageNumber);
