...
$(function () {
  ...
  $("#member_tel").bind("input propertychange", function () {
    if ($(this).val().length == 10 && $(this).val().substr(0, 2) == "09") {
      ...
      $.ajax({
        type: "POST",
        url: "https://soleystudio.000webhostapp.com/SoleyStudio/api/member/tel-UpdateCheckUni.php",
  ...
  $("#updMember_ok_btn").click(function () {
    if (flag_tel) {
      ...
      $.ajax({
        type: "POST",
        url: "https://soleystudio.000webhostapp.com/SoleyStudio/api/member/member-Update.php",
        data: JSON.stringify(dataJSON),
        dataType: "json",
  ...
  if (data.state) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        location.href = "http://192.168.10.60/project/member.html";
      }
    });
  } else {
