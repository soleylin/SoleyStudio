...
$(function () {
  ...
  $("#member_tel").bind("input propertychange", function () {
    if ($(this).val().length == 10 && $(this).val().substr(0, 2) == "09") {
      ...
      $.ajax({
        type: "POST",
        url: "https://http://soleystudio.infinityfreeapp.com/api/member/tel-UpdateCheckUni.php",
  ...
  $("#updMember_ok_btn").click(function () {
    if (flag_tel) {
      ...
      $.ajax({
        type: "POST",
        url: "https://http://soleystudio.infinityfreeapp.com/api/member/member-Update.php",
        data: JSON.stringify(dataJSON),
        dataType: "json",
