...
$(function () {
  ...
  $("#modal_done_btn").click(function () {
    ...
    if (flag_itemContent && flag_price) {
      ...
      $.ajax({
        type: "POST",
        url: "https://soleystudio.000webhostapp.com/SoleyStudio/api/manager/reserve/reserveDone-Create.php",
...
function showdataReserveDone(data) {
  if (data.state) {
    ...
    $.ajax({
      type: "POST",
      url: "https://soleystudio.000webhostapp.com/SoleyStudio/api/manager/member/memberConsumption-Update.php",
      data: JSON.stringify(dataJSON),
      dataType: "json",
  ...
  if (data.state) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        location.href = "http://192.168.10.60/project/admin/reserveList.html";
      }
    });
  } else {
    Swal.fire(data.message);
