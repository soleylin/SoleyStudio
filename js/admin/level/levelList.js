  ...
  $("body").on("click", "#mybody #delete_btn", function () {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          type: "POST",
          url: "http://https://soleystudio.000webhostapp.com/api/manager/level/level-Delete.php",
          data: JSON.stringify(dataJSON),
          dataType: "json",
          success: showdata_delete,
          error: function () {
            Swal.fire("error-- manager/level/level-Delete.php");
          },
        });
      }
    });
...
function showdata_delete(data) {
  if (data.state) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        location.href = "https://soleylin.github.io/SoleyStudio/admin/level.html";
      }
    });
  }
}
